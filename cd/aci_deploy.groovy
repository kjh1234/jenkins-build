...
//    }
    ...
    stage('Delete Old VM') {
	  ...
	  az container delete --yes --ids \$(az container list -g aci-tf-jenkins --query "[?contains(name, 'blue')].id" -o tsv)
	  az network profile delete --yes --ids \$(az network profile list --resource-group aci-tf-jenkins --query  "[?contains(name, 'blue')].id" -o tsv)
