	    ...
	    """
            ...
            for (privateIp in privateIps) {
              sh """
                ...
                # app run
                ssh -i '${identity}' -t -o StrictHostKeyChecking=no -o ProxyCommand="ssh -i '${identity}' -o StrictHostKeyChecking=no azureuser@${deployIp} nc ${privateIp} 22" azureuser@${privateIp} "java -jar ${IMAGE_NAME}-${params.TAG_VERSION}.jar"
              """
          // echo ${ip}"
            }
          }
