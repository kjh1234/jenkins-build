...
resource "azurerm_lb_rule" "green" {
  ...
  frontend_ip_configuration_name = "PublicIPAddress"
  probe_id                       = "${azurerm_lb_probe.green.id}"
}

resource "azurerm_network_profile" "green" {
  name                = "${var.prefix}-green-net-profile"
  resource_group_name = "${azurerm_resource_group.main.name}"
  location            = "${azurerm_resource_group.main.location}"
  ...
  container_network_interface {
    ...
    ip_configuration {
      name      = "${var.prefix}-green-nic-config"
      subnet_id = "${azurerm_subnet.main.id}"
    }
  }
}
...
resource "azurerm_container_group" "green" {
  name                = "${var.prefix}-green-ci"
  location            = "${azurerm_resource_group.main.location}"
  resource_group_name = "${azurerm_resource_group.main.name}"
  ip_address_type     = "Private"
  network_profile_id  = "${azurerm_network_profile.green.id}"
  os_type             = "Linux"
...
resource "azurerm_lb_backend_address_pool_address" "green" {
  name                    = "green-bepool-addr"
  backend_address_pool_id = "${azurerm_lb_backend_address_pool.green.id}"
  virtual_network_id      = "${azurerm_virtual_network.main.id}"
  ip_address              = "${azurerm_container_group.green.ip_address}"
}
